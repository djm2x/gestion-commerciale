...
import { ModelList } from './commande.component';
// pdfMake.vfs = pdfFonts.pdfMake.vfs;

@Injectable({
  providedIn: 'root'
})
export class PdfCommandeService {
  // doc https://pdfmake.github.io/docs/document-definition-object/images/
  // play arround http://pdfmake.org/playground.html
  constructor() { }

  generatePdf(model: ModelList[], format, userName, fournisseur, isEmpty = true) {

    const myBody: any[][] = [['Article', 'Qte', 'Prix', 'Montant TTC']];
    const fs = 9;
    const corps = model.forEach(e => {
      myBody.push([
        { text: e.article },
        { text: e.qte },
        { text: isEmpty ? '' : e.prixMin },
        { text: isEmpty ? '' : (+e.prixMin * +e.qte) },
      ]);
    });
          ...
          columns: [
            {
              ...
              table: {
                ...
                body: [
                  [{ text: 'Commande', style: 'tableHeader', colSpan: 2, alignment: 'left' }, {}],
                  // ['Numero', model.id],
          ...
          columns: !true ? [
            ...
            },
            {
              ...
              // },
              {
                ...
                table: {
                  body: [
                    ['Total TTC', isEmpty ? '.                 .' : model.map(e => e.prixMin * e.qte).reduce((p, c) => p + c)],
                    // ['', '']
                  ]
                }
              }
